...
# ╚══════════════════════════════════════════════════════════════╝
#
#   ./build-mac.sh              the release build: signed + notarized
#   ./build-mac.sh --quick      signed only, no notarization (fast, local)
#   ./build-mac.sh --unsigned   neither (fastest, local)
#   ./build-mac.sh --setup      re-enter the Apple credentials, then build
#
...
if [ "$SIGN" -eq 0 ]; then
  ...
  if [ -z "$IDENTITY" ]; then
    ...
    echo ""
    echo -e "  Meanwhile you can build for yourself: ${BOLD}./build-mac.sh --unsigned${NC}"
    exit 1
  fi
...
setup_credentials() {
  ...
  if xcrun notarytool store-credentials "$NOTARY_PROFILE" \
    ...
    echo -e "  Run ${BOLD}./build-mac.sh --setup${NC} to try again."
    exit 1
  fi
}
